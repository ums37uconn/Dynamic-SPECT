...
# include "spline.h"
	...
	int NDATA=20*60;// number of data points
	int NBasis=13;
	int NControlPoints=13;// number of control points


int main(){

	double *tdata;
	tdata=new double[NControlPoints];
	double *width;
	width=new double[NBasis];
	double tval=1.0;

	for (int i=0;i<NBasis;i++)width[i]=1.0/pow(i+2.0, 3.0);
	
	ofstream test("bspline.csv");
	for (int i=0;i<NControlPoints;i++) tdata[i]=(double)i;//nodes

	for (int i=0;i<NBasis;i++){
		for(int t=0;t<NDATA;t++){
			tval=width[i]*t;
			//set the output in single precision
			float spline=basis_function_b_val ( tdata, tval );
			test<<t<<"\t"<<spline<<endl;
			}
			test<<endl;
		}
...
//
  	if ( tval < tdata[1] ){
    		...
    		yval = ( ( (  - 3.0 * u + 3.0 ) * u + 3.0 ) 
                		 * u + 1.0 ) / 6.0;
  		}
 	 else if ( tval < tdata[3]){
    		yval = ( ( (     + 3.0 * u - 6.0 ) * u + 0.0 ) 
                 		* u + 4.0 ) / 6.0;
  	}
