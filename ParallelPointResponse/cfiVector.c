...

{
	...
	const short         *short_ptr;
	const long          *long_ptr;
	const float         *float_ptr;
	const double        *double_ptr;

	(void)sprintf(l_msg, "writing %ld value(s) per line", valuesPerLine);
	...
	switch (getVectorValueType(vector))
	{
	  case byteType:
		...
		(void)fprintf(stream, "%ld byteType [", valueCount);
	  ...
	  case shortType:
		...
		(void)fprintf(stream, "%ld shortType [", valueCount);

		if ( ( valuesPerLine > 0 )
		&&   ( void_ptr != (void *)NULL ) )
		{
			short_ptr = (short *)void_ptr;
			for ( i = 0; i < valueCount; i++ )
			{
				if ( (i % valuesPerLine) == 0 )
				{
	  ...
	  case longType:
		...
		(void)fprintf(stream, "%ld longType [", valueCount);
	  ...
	  case floatType:
		...
		(void)fprintf(stream, "%ld floatType [", valueCount);
	  ...
	  case doubleType:
		...
		(void)fprintf(stream, "%ld doubleType [", valueCount);
	  ...
	  default:
		...
		(void)fprintf(stream, "%ld unknownType [", valueCount);
		(void)fprintf(stream, "\n]\n");
		break;
...

{
	...
	(void)sprintf(l_msg, "allocating %ld value(s)", valueCount);
	...
	switch (valueType)
	{
	  ...
	  case shortType:
		...
		DEBUG_MSG(cfiMemoryDebug, "...of type short");
		memorySize = valueCount * sizeof(short);
		break;
...

{
	...
	switch (vector->type)
	{
	  ...
	  case shortType:
		...
		(*value_ptr) = (cfiScalar)(*(((short *)void_ptr) + index));
		return (cfiSuccess);
...

{
	...
	switch (vector->type)
	{
	  ...
	  case shortType:
		...
		*(((short *)void_ptr) + index) = (short)value;
		return (cfiSuccess);
...
/* LINTED */
	...
	switch (vector->type)
	{
	  ...
	  case shortType:
		...
		return ((void *)(((short *)vector->value) + index));
