...

{
	...
	(void)sprintf(l_msg, "allocating a %d-D array", axisCount);
...
/* LINTED */
	...
	else
	{
		...
		for ( i = 0; i < axisCount; i++ )
		{
			...
			if ( i == 0 )
			{
				(void)sprintf(l_msg, "...with dimensions %d", dimension);
			}
			else
			{
				(void)sprintf(l_msg, "%sx%d", l_msg, dimension);
			}
...

{
	...
	{
		(void)fprintf(stream, " %d", (array->dimensions)[i]);
	}
	(void) fprintf(stream, "\n]\n");
