...
int read_paramaters(int parm_num, char escape, double *params, FILE *fptr)
{
  int i=0,j,buf_pos=0;
...
if((argc != 3) && (argc != 4))
	...
	//cfiCounter ANGLE_COUNT =1;// (cfiCounter)(params[0]);

	//cfiScalar ANGLE_START = (270+params[1])/360;
	...
	cfiScalar ATTENUATION_CUT_OFF=params[24];

	/*  allocate and read the attenuation map */
	...
	if(argc == 4)//with attenuation map
		{
			...
			//fprintf(stdout, "reading %s\n", ATN_FILE_NAME);
				...
				{
				FAILURE_MSG( "Cannot read attenuation map file \n" );
				(void)exit(cfiFailure);
				}
        ...
        // rewind file pointer
         //rewind( file_ptr );
       	...
       	int numberOfAttenuationMapVoxelsRead 
		= fread( atnMeshValue, sizeof( cfiScalar ), atn_voxel_count, file_ptr );
/////////////if the atn map is in float format change it ////////////////////////////////////////////////////////////////////////////////////////////
		//= fread( atnMeshValue, sizeof( float ), atn_voxel_count, file_ptr );

	if ( numberOfAttenuationMapVoxelsRead != atn_voxel_count )
				{
		   	 	fprintf(stderr, "Read number of attenuation map voxels: %d\n", numberOfAttenuationMapVoxelsRead );
		   	 	fprintf(stderr, "Expected number of attenuation map voxels: %d\n", atn_voxel_count );
				FAILURE_MSG( "Mismatch between actual and expected number of attenuation map voxels\n" );
				(void)exit(cfiFailure);
				}
	...
	else {//without attentuation map
		/*memset(atnMeshValue, 0, atn_voxel_count * sizeof(cfiScalar));*/
		for(index = 0; index<atn_voxel_count; index++)
			atnMeshValue[index] = 1.0e-020;
		}

//set amap value to zero
for(index = 0; index<atn_voxel_count; index++){
if(atnMeshValue[index]<ATTENUATION_CUT_OFF)atnMeshValue[index]=0.0;
//if(atnMeshValue[index]<0.120&&atnMeshValue[index]>0.060)atnMeshValue[index]=0.0;
}
//fprintf(stderr,"%d   %f\n",index, atnMeshValue[index]);
	...
	//fprintf(stderr, "\nray_sample_count = %ld\n", ray_sample_count);
	//fprintf(stderr, "ptresp_count = %ld\n\n", ptresp_count);
	...
	for ( prow = 0; prow < ptresp_count; prow++ )
		{
	    	for ( pcol = 0; pcol < ptresp_count; pcol++ )
	   	 	{
			rT = sqrt(((prow - (ptresp_count / 2)) * (prow - (ptresp_count / 2))) 
			+ ((pcol - (ptresp_count / 2)) * (pcol - (ptresp_count / 2)))) * ptresp_bin_width;
	...
	cfiScalar angleInRadians, angleInDegrees;

	for ( angle = 0; angle < ANGLE_COUNT; angle++ )//angle for loop
		{
		...
		angleInRadians = 2.0 * M_PI * ( ANGLE_START + ((cfiScalar)(angle*ANGLE_MODIFIER*ANGLE_DIRECTION)) / ((cfiScalar)ANGLE_COUNT ) );
        	angleInDegrees = angleInRadians * 180.0 / M_PI;

	  	radial_dx = cos( angleInRadians );
	  	radial_dy = sin( angleInRadians );
	  	radial_dz = 0.0;

	  	//fprintf(stderr,"dx: %.2f  dy: %.2f .an: %d .an (deg): %.2f  | ", radial_dx, radial_dy, angle, angleInDegrees);
	  	...
	  	//fprintf(stderr,"x: %.1f  y: %.1f z: %.1f\n",detector_origin_x,detector_origin_y,detector_origin_z);


		for ( drow = 0; drow < DETECTOR_Z_COUNT; drow++ )//detector row loop
	  		{
	    			//fprintf(stderr, ".");
			...
			for ( dcol = 0; dcol < DETECTOR_XY_COUNT; dcol++ )//detector column loop
	    				{
		...
		for ( prow = 0; prow < ptresp_count; prow++ )//pixel row
	      			{
					ptresp_z = ptresp_origin_z + (prow * ptresp_dz);
					ray_dz = ptresp_z - detector_z;

			for ( pcol = 0; pcol < ptresp_count; pcol++ )//pixel column
				{
		 	 	/*
		...
		for ( ; ray_ptr < end_ray; ray_ptr++, atn_ptr++ )
			...
			Fcol0 = ((cfiCounter)index_ptr[0]) + (((cfiCounter)index_ptr[1]) * VOXEL_X_COUNT) 
			+ (((cfiCounter)floor(index_ptr[2])) * voxel_xy_count);
