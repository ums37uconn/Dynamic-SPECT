...
// --- functions.  This concains everything we need except for nrutil functions
int main(int argc, char **argv);
void MLEM_recon(int Nsinogram, float *sinogram, int Nvolume, float *volume, char *fname, int LastIteration, int OutputNumber, int *IterationsForOutput, char *image_fname);
...
void TB_MLEM_recon(int Nsinogram, int Nvolume, int singleProjectionSize, int basisNum, int basisLen, float *sinogram, float *coeffVolume, float **basis, char *sysmat_fname, int LastIteration, int OutputNumber, int *IterationsForOutput, char *image_fname);
void TimeBasisBP(int Nsinogram, int Nvolume, int singleProjectionSize, int basisNum, int basisLen, float *sinogram, float *coeffVolume, float **basis, char *sysmat_fname);
void TimeBasisFP(int Nsinogram, int Nvolume, int singleProjectionSize, int basisNum, int basisLen, float *sinogram, float *coeffVolume, float **basis, char *sysmat_fname);
...
int main(int argc, char **argv)
{
  ...
  int maxNumberOfArguments = 8;

  //
  if((argc < minNumberOfArguments ) || (argc > maxNumberOfArguments + MAX_OUTPUTS))
  {
    fprintf(stderr, "\nUsage:\n%s sysmat.file sinogram.file image.file operation n_iterations {timebasis.file SingleProjectionSize} (Iteration numbers for output)\n",argv[0]);
    fprintf(stderr, "Where \"operation\" can be either R for reconstruction, or F or B for forward or backprojection (CAPITAL LETTERS).\n");
    fprintf(stderr, "Or r\\f\\b (non capital letters) for these operations with a time basis file {following should be a time basis file, and the number of data points or projection from a single angle}.\n");
    fprintf(stderr, "(Iteration numbers can be one or more number of iterations below %d, such as: 10 25 100.)\n",MAX_ITERATIONS);
    exit(1);
  }
  ...
  switch(Action)
  {
    ...
    case 'f': Action = 'f'; tbFlag = 1; break;//forward  with time  basis on
    case 'F': Action = 'f'; tbFlag = 0; break;//forward with time basis off
    case 'b': Action = 'b'; tbFlag = 1; break;//backward with time basis on
    case 'B': Action = 'b'; tbFlag = 0; break;//backward with time basis off
    default:
...
//fprintf( stderr, "Nvolume=%d \n", Nvolume );


  // -- if using time basis file, than read it
  if ( tbFlag ) {
	  ...
	  if (argc > 6)
	  {
		  ...
		  {
			fprintf(stderr, "SingleProjectionSize %d must be an integer smaller than the size of the original sinogram %d.\n",singleProjectionSize,Nsinogram);
			exit(1);
		  }
	  }
	  else
	  {
		fprintf(stderr, "After operation r\\f\\b should come the time basis file, and following the number of data points of projection from a single angle \n");
		exit(1);
	  }
...
// at this point, we have read all the required input data, except sysmat
  ...
  switch(Action)
  {
   ...
   //   fprintf(stderr, "Reconstruction volume of length %d from sinogram of length %d\n", Nvolume, Nsinogram);
      if(tbFlag)
	      TB_MLEM_recon(Nsinogram, Nvolume, singleProjectionSize, tbNum, tbLen, sinogram, volume, basis, sysmat_fname, LastIteration, OutputNumber, IterationsForOutput, image_fname);
      else
	      MLEM_recon(Nsinogram, sinogram, Nvolume, volume, sysmat_fname, LastIteration, OutputNumber, IterationsForOutput, image_fname);
      break;
      
    case 'f':
      fprintf(stderr, "Forward project volume of length %d into sinogram of length %d\n", Nvolume, Nsinogram);
      if(tbFlag)
	    TimeBasisFP(Nsinogram, Nvolume, singleProjectionSize, tbNum, tbLen, sinogram, volume, basis, sysmat_fname);
      else
    ...
    case 'b':
      ...
      if(tbFlag)
	    TimeBasisBP(Nsinogram, Nvolume, singleProjectionSize, tbNum, tbLen, sinogram, volume, basis, sysmat_fname);
      else
...
float *volume_old;
  ...
  for(n=0; n<Nvolume; n++) volume[n] = 1.;
...
void TB_MLEM_recon(int Nsinogram, int Nvolume, int singleProjectionSize, int basisNum, int basisLen, float *sinogram, float *coeffVolume, float **basis, char *sysmat_fname, int LastIteration, int OutputNumber, int *IterationsForOutput, char *image_fname)
{
  ...
  char temp_fname[128];
  // --- initial volume assignment: all pixels are one
  for(n=0; n<Nvolume; n++) coeffVolume[n] = 1.;
...
// --- compute  element-by-element inverse of efficiency matrix
  for(n=0; n<Nsinogram; n++) tmp_sinogram[n] = 1.;
  	TimeBasisBP(Nsinogram, Nvolume, singleProjectionSize, basisNum, basisLen, tmp_sinogram, constant_denominator, basis, sysmat_fname);
...
//  -------- ITERATION LOOP --------
  for(niter=1; niter<=LastIteration; niter++)
  {
    //fprintf(stderr, "Iteration No %d of %d\n", niter, LastIteration);
// compute the reprojection through the n-1 version of the file into tmp_sinogram
    TimeBasisFP(Nsinogram, Nvolume, singleProjectionSize, basisNum, basisLen, tmp_sinogram, coeffVolume, basis, sysmat_fname);
...
// backproject the result into tmp_volume
      TimeBasisBP(Nsinogram, Nvolume, singleProjectionSize, basisNum, basisLen, tmp_sinogram, tmp_volume, basis, sysmat_fname);
    ...
    for(rotationIndex=0;rotationIndex<rotationNum;rotationIndex++)
	    /* If this time point exists, compute the value for the sinogram */
		    ...
		    if(fabs(sinogram[ns+rotationInSinogram[rotationIndex]])>1.0e-14)
			    for(n=0;n<Nchunk;n++)
				    for(basisIndex=0;basisIndex<basisNum;basisIndex++)
					    coeffVolume[Itemp[n]+basisInVolume[basisIndex]] += sinogram[ns+rotationInSinogram[rotationIndex]] * basis[basisIndex][singleProjInBasis[rotationIndex][currentProjection]] * SMtemp[n];
	    ...
	    for(basisIndex=0;basisIndex<basisNum;basisIndex++)
		   ...
		   if(fabs(coeffVolume[Itemp[n]+basisInVolume[basisIndex]])>1.0e-14)
			    for(rotationIndex=0;rotationIndex<rotationNum;rotationIndex++)
				    ...
				    /* If this time point exists, compute the value for the sinogram */
				    if(singleProjFlag[rotationIndex][currentProjection])
					    sinogram[ns+rotationInSinogram[rotationIndex]] += coeffVolume[Itemp[n]+basisInVolume[basisIndex]] * basis[basisIndex][singleProjInBasis[rotationIndex][currentProjection]] * SMtemp[n];
			    }
